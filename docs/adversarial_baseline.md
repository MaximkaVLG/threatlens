...
**Status:** v2 results final.
python_only adversarial grid was started but stopped before completion
(operator needed to power off the laptop; resuming the grid would take
~3 hr more). The partial python_only data confirmed cell 1 (iat_jitter
mild → 60.29 % recall, baseline 60.46 %, Δ −0.17 pp) — consistent with
v2's iat_jitter robustness — but a full head-to-head matrix is left
for a follow-up run.

For Yandex Practicum Award purposes, v2 is the production candidate, so
its adversarial floor is the figure that matters. The python_only floor
is a "previous-prod for-the-record" comparison; we'll add it when the
laptop is up long enough to finish the run.
...
## Results — python_only (current production) — partial

The python_only grid was stopped after **1 of 12 active cells** completed:

| Perturbation | strength | recall | Δ vs baseline |
|---|---|---:|---:|
| iat_jitter | mild | 60.29 % | −0.17 pp |

(Baseline is 60.46 %, n=349, matches the bootstrap CI in
[`results/python_only/bootstrap_ci.json`](../results/python_only/bootstrap_ci.json)
exactly.)

That single cell rhymes with v2's behaviour on the same perturbation
(also ~0 effect — see the v2 row in the table above), so we have
weak evidence that timing-jitter is not a productive evasion technique
against either model. We don't claim more than this from one cell.

A full head-to-head requires running the remaining 11 active cells
(`packet_padding`, `ttl_random`, `rst_inject` × 3 strengths plus the
remaining `iat_jitter` strengths). Wall time on a laptop is ~3 hours.
The script + comparison generator are committed in this PR
(`scripts/adversarial_eval.py`, `scripts/adversarial_compare.py`); the
data lands when the grid is rerun.

## Head-to-head (v2 vs python_only under perturbation)

Deferred — see python_only section above. Auto-generation logic is
shipped in [`scripts/adversarial_compare.py`](../scripts/adversarial_compare.py);
running it on the full grid output produces
[`docs/adversarial_compare.md`](adversarial_compare.md) with side-by-side
recall + delta-vs-baseline + cell-level differences.
